...
public class TDTJava {
    ...
    public static double trackingAdaptThreshould = 0.5;
    public static double topicDetectionThreshold = /*0.12;*/0.02;
    public static double topicDetectionAdaptThreshold = /*0.3;*/0.1;
    ...
    public static void main(String[] args) throws Exception{
    	...
    	testTopicDetection(400, 1, 1);
    	//IdfExternalProvider idf = new IdfExternalProvider();
//    	List<TfidfVectorSpaceDocumentRepresentation> docs = getDateSortedAndFilteredStoryVectorsAndBuildIdf(false, idf, 350);
//    	DEBUG=true;
//    	trackingThreshold=0.15;
//    	Result result = testTopicTrackingDynamically(docs, idf, "5", 4, true, 2.0, 1.0);
    }
    ...
    public static Result testTopicDetection(int filterLimit,double C_miss,double C_false_alarm) throws Exception{
        ...
        for(int batchNumber=0;i<docVectors.size();batchNumber++){
	        ...
	        for(int k=0,c=i-bufferSize;k<buffer.size();k++,c++){
	        	...
	        	if(useClosestStoryTopicNotAverageSimilarity){
		        	double maxSim=0;
		        	int bestMatch=-1;
		        	...
		        	// if our system decided the new story is similar to a previously seen one
		        	if(ourTopics.size()>0 && maxSim>topicDetectionThreshold){
		        		// get the topic of the similar story
			        	ourTopic = topicMap.get(c_docVectors.get(bestMatch));
			        	ourTopic.addStoryToTopic(currentDocument.getStoryFile().getStoryTag(), currentDocument);
	        		...
	        		for(int topic=0;topic<ourTopics.size();topic++)
	        		{
	        			...
	        			for(TfidfVectorSpaceDocumentRepresentation story:ourTopics.get(topic).stories)
      ...
      System.out.println("Our model's number of clusters is " + ourTopics.size() + " and the actual is " + seenTopics.size());
      for(i=0;i<ourTopics.size();i++){
    	  System.out.println("Topic " + i + " : ");
    	  for(TfidfVectorSpaceDocumentRepresentation s:ourTopics.get(i).stories){
    		  System.out.println(s.getStoryFile().getStoryTag() + "\t\t: " + s.getStoryFile().getStoryTitle());
    	  }
    	  System.out.println("====================================");
      }
        ...
        Result result = new Result(total_misses,total_false_alarms,total_actual_num_of_new,docVectors.size());
        //result.setCost( 
    ...
    public static void testDetection(int drawChart) throws Exception{
        ...
        for(int test=0;test<TESTS;test++)
        {
	        ...
	        for(double threshold : TDTJava.thresholds){
	        	...
	        	int cntTakenTopic=0;
	            Result result = testFirstStoryDetectionDynamically(docVectors, idfProvider, filterLimit, 1,2); 
	        	if(result==null)
	        		continue;
	        	cntTakenTopic++;
	        	...
	        	System.out.println("Th=" + threshold + " - " + "Miss: " + result.numberOfMisses + "(" + result.getMissRate() + ")"
        ...
        if(drawChart == DRAW_CHART_COST || drawChart==DRAW_CHART_BOTH){
	        ...
	        if(TESTS>1)
	        	linear.drawInLinearScale(thresholds[1], costs[1], "cost-no-normalization");
    ...
    public static Result testFirstStoryDetectionDynamically(List<TfidfVectorSpaceDocumentRepresentation> docVectors, IdfExternalProvider idfProvider,
    		int filterLimit,double C_miss,double C_false_alarm) throws Exception{
    ...
    public static void testTracking(int drawChart) throws Exception{
        ...
        for(int test=0;test<TESTS;test++)
        {
	        ...
	        for(double threshold : TDTJava.thresholds){
	            ...
	            {
	            	Result result = testTopicTrackingDynamically(docVectors,idfProvider, topicName, numOfTrainingDocs, shuffle_training_stories, 2, 1); 
